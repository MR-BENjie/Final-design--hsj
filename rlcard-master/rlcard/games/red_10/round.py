...
'''
#the player get the card red 10 is landlord
import functools
import numpy as np

from rlcard.games.red_10 import Dealer
from rlcard.games.red_10.utils import cards2str, doudizhu_sort_card
from rlcard.games.red_10.utils import CARD_RANK_STR, CARD_RANK_STR_INDEX


class Red_10Round:
    ''' Round can call other Classes' functions to keep the game running
    '''
    def __init__(self, np_random, played_cards, played_colors):
        self.np_random = np_random
        self.played_cards = played_cards
        self.played_colors= played_colors
        self.trace = []
    ...
    def update_public(self, action, color):
        ''' Update public trace and played cards

        Args:
            action(str): string of legal specific action
        '''
        self.trace.append((self.current_player, action, color))
        if action != 'pass':
            for c in action:
            ...
            if color!=None:
                for co in color:
                    if co == "S":
                        self.played_colors[self.current_player][0] = 1
                    elif co == "H":
                        self.played_colors[self.current_player][1] = 1
                    elif co == "D":
                        self.played_colors[self.current_player][2] = 1
                    elif co == "C":
                        self.played_colors[self.current_player][3] = 1
            self.public['played_cards'] = self.cards_ndarray_to_str(self.played_cards)
            self.public["played_red_10_color"] = self.colors_to_str(self.played_colors)

    def colors_to_str(self,colors):
        colorlist = ["S", "H", "D", "C"]
        resultlist= []
        for p in colors:
            finalstr = ""
            for index,x in enumerate(p):
                if x == 1:
                    finalstr+=colorlist[index]
            resultlist.append(finalstr)
        return resultlist
    def proceed_round(self, player, action, color):
        ...
        '''
        self.update_public(action, color)
        self.greater_player = player.play(action, self.greater_player, color)
