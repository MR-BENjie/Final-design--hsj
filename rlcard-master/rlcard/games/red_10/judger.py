...
class Red_10Judger:
    ...
    def playable_cards_from_hand(current_hand):
        ...
        for i in more_than_3_indexes:
            ...
            for left, right in Red_10Judger.solo_attachments(current_hand, i[0], 1, 2):
                pre_attached = ''
                for j in left:
            ...
            for left, right in Red_10Judger.pair_attachments(cards_count, i[0], 1, 2):
                pre_attached = ''
                for j in left:
        ...
        solo_chain_indexes = Red_10Judger.chain_indexes(non_zero_indexes)
        ...
        pair_chain_indexes = Red_10Judger.chain_indexes(more_than_1_indexes)
        ...
        trio_chain_indexes = Red_10Judger.chain_indexes(more_than_2_indexes)
            ...
            while(l >= 2):
                ...
                while (curr_length < l and curr_length < 6):
                    ...
                    if (curr_length >= 2 and curr_length <= 5):
                        for left, right in Red_10Judger.solo_attachments(current_hand, s, curr_length, curr_length):
                            pre_attached = ''
                            for j in left:
                                pre_attached += CARD_RANK_STR[j]
                            post_attached = ''
                    ...
                    if (curr_length >= 2 and curr_length <= 4):
                        for left, right in Red_10Judger.pair_attachments(cards_count, s, curr_length, curr_length):
                            pre_attached = ''
                            for j in left:
                                pre_attached += CARD_RANK_STR[j] * 2
                            post_attached = ''
    ...
    def judge_payoffs(landlord_id, winner_id):
        ...
        if winner_id in landlord_id:
            for index, _ in enumerate(payoffs):
                if index in landlord_id:
                    payoffs[index] = 1
        else:
            for index, _ in enumerate(payoffs):
                if index not in landlord_id:
                    payoffs[index] = 1
        return payoffs
