...
from rlcard.games.red_10.utils import cards2str, doudizhu_sort_card, CARD_RANK_STR
from rlcard.games.red_10 import Player
from rlcard.games.red_10 import Round
from rlcard.games.red_10 import Judger
...
class Red_10Game:
    ...
    def init_game(self):
        ...
        self.played_colors = [np.zeros((4, ), dtype=np.int)
                                for _ in range(self.num_players)]
        self.round = Round(self.np_random, self.played_cards, self.played_colors)
    ...
    def step(self, action, color=None):
        ...
        self.round.proceed_round(player, action, color)
        if (action != 'pass'):
    ...
    def _get_others_current_hand(self, player):
        ...
        player_down_2 = self.players[(player.player_id-2) % len(self.players)]
        others_hand = merge(player_up.current_hand, player_down.current_hand, key=functools.cmp_to_key(doudizhu_sort_card))
        others_hand = merge(others_hand , player_down_2.current_hand, key=functools.cmp_to_key(doudizhu_sort_card))
