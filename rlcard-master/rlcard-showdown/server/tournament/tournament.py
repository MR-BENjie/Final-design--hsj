    ...
    def launch(self):
        ...
        for i in range(num_models):
            for j in range(num_models):
                ...
                if self.game == 'red_10':
                    agent = self.models[i].agents()
                    agents = [agent[0], agent[1], agent[2],agent[3]]
                    names = [self.model_ids[i], self.model_ids[j], self.model_ids[j],self.model_ids[j]]
                    data, payoffs, wins = red_10_tournament(self.game, agents, names, self.num_eval_games,self.models[i])
...
def red_10_tournament(game, agents, names, num_eval_games, game_model):
    ...
    for _ in tqdm(range(num_eval_games)):
        ...
        data['moveHistory'] = []
        while not env.is_over():
            relation_score, dangerou_score,model_numbel, agent = game_model.agents(rela_state,danger_state)
