...
class Red10GameBoard extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        const leftIdx = bottomIdx >= 0 ? (bottomIdx + 3) % 4 : -1;
        const frontIdx =  bottomIdx >= 0 ? (bottomIdx + 2) % 4 : -1;
        let rightId = -1;
        let leftId = -1;
        let frontId = -1;

        if (rightIdx >= 0 && leftIdx >= 0 && frontIdx >=0) {
            ...
            found = this.props.playerInfo.find((element) => {
                return element.index === frontIdx;
            });
            if (found) frontId = found.id;
        }
        return (
            <div
                className="red10-wrapper"
                onMouseLeave={(e) => {
                    e.stopPropagation();
            ...
            >
                ...
                >
                    ...
                    <div id={'front-player'}>
                        <div className="player-main-area">
                            <div className="player-info">{this.computePlayerPortrait(frontId, frontIdx)}</div>
                            {frontIdx >= 0 ? (
                                this.computeSideHand(this.props.hands[frontIdx])
                            ) : (
                                <div className="player-hand-placeholder">
                                    <span>{t('waiting...')}</span>
                                </div>
                        ...
                        <div className="played-card-area">{frontIdx >= 0 ? this.playerDecisionArea(frontIdx) : ''}</div>
                    </div>
...
export default withTranslation()(Red10GameBoard);
