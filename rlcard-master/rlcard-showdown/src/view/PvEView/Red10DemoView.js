...
let initHands = [
    shuffledDoudizhuDeck.slice(0, 13),
    shuffledDoudizhuDeck.slice(13, 26),
    shuffledDoudizhuDeck.slice(26, 39),
    shuffledDoudizhuDeck.slice(39, 52),
];
...
function PvERed10DemoView() {
    ...
    const proceedNextTurn = async (playingCard, rankOnly = true) => {
        ...
        if (playerInfo[gameState.currentPlayer].index === 0 && threeLandlordCards.length > 0) {
            let playingCardCopy;
        ...
        if (newHand.length === 0) {
            ...
            switch (playerInfo[mainPlayerId].douzeroPlayerPosition) {
                ...
                case 2:
                    ...
                    if (winner.role === playerInfo[mainPlayerId].role) {
                        gameStatistics.totalWinNum += 1;
                        gameStatistics.landlordFrontWinNum += 1;
                    }
                    break;
                case 3:
    ...
    const requestApiPlay = async () => {
        ...
        const other_hand_cards = cardArr2DouzeroFormat(
            sortDoudizhuCards(
                ...
                )).concat(gameState.hands[(gameState.currentPlayer + 3) % 4],),
                true,
            ),
        );
    ...
    return (
        <div>
            ...
            >
                ...
                <DialogContent>
                    <TableContainer className="red10-statistic-table" component={Paper}>
...
export default PvERed10DemoView;
