...
import { Red10GameBoard } from '../../components/GameBoard';
import {
    card2SuiteAndRank,
...
var flag = 'a';
class Red10ReplayView extends React.Component {
    constructor(props) {
        super(props);
    ...
    generateNewState() {
        ...
        if (this.state.gameInfo.turn + 1 < this.gameStateHistory.length) {
            ...
            if (newMove.playerIdx === this.state.gameInfo.currentPlayer) {
                ...
                gameInfo.turn++;
                gameInfo.currentPlayer = (gameInfo.currentPlayer + 1) % 4;
    ...
    startReplay() {
        ...
        axios
            ...
            .then((res) => {  
                ...
                flag = gameInfo.playerInfo[0].agentInfo
            ...
            .catch(() => {
                ...
                Message({
                    message: 'Error in getting replay data'+'|'+flag,
                    type: 'error',
                    showClose: true,
                });
            });
    ...
    computeProbabilityItem(idx, type_p) {
        ...
        if (this.state.gameInfo.gameStatus !== 'ready' && this.state.gameInfo.turn < this.moveHistory.length) {
            ...
            if (currentMove) {
                ...
                else if (type_p ==='relation'){
                  if ('relation_score' in currentMove.info)
                  {
                    probabilityItemType = 'Relation_Score';
                    probabilities = currentMove.info.relation_score;
                    } else {
                        probabilityItemType = 'Rule';
                    }  
                }
                else if (type_p ==='dangerous'){
                    if ('dangerou_score' in currentMove.info)
                    {
                      probabilityItemType = 'Dangerou_Score';
                      probabilities = currentMove.info.dangerou_score;
                      } else {
                          probabilityItemType = 'Rule';
                      }  
                  }
                        ...
                        <div className={'non-card ' + this.state.gameInfo.toggleFade}>
                            <span>
                                {probabilityItemType === 'Rule'
                                    ? 'Rule Based'
                                    : probabilityItemType === 'Relation_Score'
                                    ? `Relation_Score UP:${(probabilities[0] * 100).toFixed(2)}% FRONT:${(probabilities[1] * 100).toFixed(2)}% DOWN:${(probabilities[2] * 100).toFixed(2)}%`
                                    : `Dangerou_Score ${(probabilities * 100).toFixed(2)}%`}
                            </span>
                        </div>
                    ) : (
                        ''
    ...
    render() {
        ...
        return (
            <div>
                ...
                <div className={'red10-view-container'}>
                    <Layout.Row style={{ height: '540px' }}>
                        <Layout.Col style={{ height: '100%' }} span="17">
                            <div style={{ height: '100%' }}>
                                <Paper className={'red10-gameboard-paper'} elevation={3}>
                                    <Red10GameBoard
                        ...
                        <Layout.Col span="7" style={{ height: '100%' }}>
                            <Paper className={'red10-probability-paper'} elevation={3}>
                                ...
                                <div className={'probability-table'}>
                                    <div className={'probability-item'}>{this.computeProbabilityItem(0,'card')}</div>
                                    <div className={'probability-item'}>{this.computeProbabilityItem(1,'card')}</div>
                                    <div className={'probability-item'}>{this.computeProbabilityItem(2,'card')}</div>
                                    <div className={'probability-item'}>{this.computeProbabilityItem(0,'relation')}</div>
                                    <div className={'probability-item'}>{this.computeProbabilityItem(0,'dangerous')}</div>
                                </div>
                            </Paper>
                        </Layout.Col>
                    </Layout.Row>
                    ...
                    {(
                        <>
                            <p style={{ margin: '4px', marginTop: '15px' }}>
                                This game is based on the interface {' '}
                                <a href="https://github.com/datamllab/rlcard" target="_blank">
                                    RLCard 
                                </a>{' '}
                                . We create red10 game environment:
                                <a href="https://github.com/MR-BENjie/URL/tree/x21/URL/Red-10/douzero/env/rlcard_red10" target="_blank">
                                    RED-10
                                </a>{' '}
                                ...
                                . We create three models include cards playing policy ,relation score and dangerous score model and relation training and eval project  
                                <a href="https://github.com/MR-BENjie/URL/tree/x21/URL/Red-10"target="_blank">
                                    Model
                                </a>
                                . Gui is based one the{' '}
                                <a href="https://github.com/datamllab/rlcard-showdown" target="_blank">
                                rlcard-showdown
                                </a>
                                . We create Red_10 game GUI    {' '}
                                <a href="https://github.com/MR-BENjie/URL/tree/x21/URL/rlcard-master/rlcard-showdown" target="_blank">
                                    RED-10 GUI
                                </a>
                            </p>
                        </>
...
export default Red10ReplayView;
